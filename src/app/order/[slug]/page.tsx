...
export async function generateMetadata({ params }: OrderPageProps): Promise<Metadata> {
  ...
  return {
    title: `Order ${product.title} | PrintMaania`,
    description: `Place your order for ${product.title} - ${product.shortDescription}`,
  };
}
...
export default async function OrderPage({ params }: OrderPageProps) {
  ...
  return (
    <main className="min-h-screen bg-[#F8F8F8] overflow-x-hidden">
      ...
      <div className="bg-white border-b border-[#E5E7EB]">
        <div className="mx-auto max-w-7xl px-4 py-4">
          <nav className="flex items-center gap-2 text-sm">
            ...
            <Link href="/order" className="text-[#555555] hover:text-[#222831] transition">
              Order
            </Link>
            <ChevronRight className="h-4 w-4 text-[#555555]" />
            <span className="text-[#222831] font-medium">{product.title}</span>
          </nav>
        </div>
      </div>
